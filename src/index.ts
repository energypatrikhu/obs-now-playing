...
app.locals.ytPlayingCurrent = {
	reAlert: moment().add(60, 'seconds').unix() * 1000,
	artist: 'No Artist',
	track: 'No Track',
	thumbnail:
...
app.locals.io.on('connection', (socket) => {
	socket.emit('nowPlaying', app.locals.ytPlayingCurrent);
});

server.listen(PORT, () => {
	let msg = `OBS Now Playing App is running @ http://127.0.0.1:${PORT}`;
	msg += `\nPress CTRL+C or close this window to stop the App`;
	msg += `\n\n`;
	msg += `\n> = = = = = = = = = = = < Usage : OBS > = = = = = = = = = = = <`;
	msg += `\n > You only need to do this once!`;
	msg += `\n - Open your OBS Studio`;
	msg += `\n - Add a new Browser Source`;
	msg += `\n - Set URL to http://127.0.0.1:${PORT}/`;
	msg += `\n - Set Width to your monitor width`;
	msg += `\n - Set Height to 96 px`;
	msg += `\n - Click OK`;
	msg += `\n - Done!`;
	msg += `\n> = = = = = = = = = = = < Usage : OBS > = = = = = = = = = = = <`;
